...
public class DiskLruCache {
    ...
    private static final int MAX_CACHE_DAYS = 3;
    ...
    private long maxCacheByteSize = ImageCache.DEFAULT_MEM_CACHE_SIZE;
    ...
    private void init() {
    	...
    	if(mCacheDir.exists()) {
    		...
    		for(File f:files) {
    	        ...
    	        String key = getKeyFromPath(f);
    	        LogHelper.d(" testing "," testing file "+f.lastModified()+"");
    	        mLinkedHashMap.put(key, f.getAbsolutePath());	        
    		}
    	}
    ...
    public void put(String key, Bitmap data) {
        	if (!mLinkedHashMap.containsKey(key)) {
                try {
                	...
                	LogHelper.e(TAG, "Error in put: " + e.getMessage());
    ...
    private void flushCache() {
        ...
        while (count < MAX_REMOVALS &&
            ...
            if (BuildConfig.DEBUG) {
            	LogHelper.d(TAG, "flushCache - Removed cache file, " + eldestFile + ", "
                        + eldestFileSize);
            }
        }
    }
    
    public void purgeStaleDiskCache() {
        File eldestFile;

        ArrayList<String> keys = new ArrayList<String>(mLinkedHashMap.keySet());
        for(int i=0;i<keys.size();i++) {
            eldestFile = new File(mLinkedHashMap.get(keys.get(i)));
            long timeElapsed = Math.round((System.currentTimeMillis()-eldestFile.lastModified())/(1000.0*60.0*60.0));
            LogHelper.d(" testing"," testing eldest entry key: "+keys.get(i)+" value: "+mLinkedHashMap.get(keys.get(i))+" time elapsed: "+timeElapsed);
            
            if(timeElapsed>=(24*MAX_CACHE_DAYS)) {
            	mLinkedHashMap.remove(keys.get(i));
            	eldestFile.delete();
            	if (BuildConfig.DEBUG) {
            		LogHelper.d(TAG, " testing flushCache - Removed cache file, " + eldestFile);
            	}
            }
        }
    }
    ...
    public Bitmap get(String key) {
        synchronized (mLinkedHashMap) {
            ...
            if (file != null) {
                if (BuildConfig.DEBUG) {
                	LogHelper.d(TAG, "Disk cache hit");
                }
                ...
                if (new File(existingFile).exists()) {
                    if (BuildConfig.DEBUG) {
                    	LogHelper.d(TAG, "Disk cache hit (existing file)");
                    }
                    Bitmap bitmap = BitmapFactory.decodeFile(existingFile);
    ...
    public boolean containsKey(String key) {
        ...
        //LogHelper.d(" testing"," testing hashMap "+ mLinkedHashMap.toString());
        //LogHelper.d(" testing"," testing key "+ key);

    	if (mLinkedHashMap.containsKey(key)) {
        	return true;
        }
    	LogHelper.d(" testing"," testing hash map not contained ");    	
    ...
    public void clearCache() {
        ...
        if(mLinkedHashMap != null) mLinkedHashMap.clear();
    }
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        if(Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED || !ImageLoaderUtils.isExternalStorageRemovable() && ImageLoaderUtils.getExternalCacheDir(context)!=null) {
        	cachePath = ImageLoaderUtils.getExternalCacheDir(context).getPath();
        } else {
    ...
    public static String createFilePath(File cacheDir, String key) {
        try {
        	...
        	LogHelper.e(TAG, "createFilePath - " + e);
        }

        return null;
    }
    
    public static String getKeyFromPath(File cacheDir) {
        try {
            // Use URLEncoder to ensure we have a valid filename, a tad hacky but it will do for
            // this example
        	return URLDecoder.decode(cacheDir.getName().replaceFirst(CACHE_FILENAME_PREFIX, ""),"UTF-8");
        } catch (final UnsupportedEncodingException e) {
        	LogHelper.e(TAG, "createFilePath - " + e);
        }
